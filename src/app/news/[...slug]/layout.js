...
export async function generateMetadata({ params }) {
  try {
    ...
    const {
      seoTitle = "خبر",
      seoDescription = "توضیحات خبر",
      seoKeywords = "اخبار, تازه‌ها, سایت خبری",
      seoHeadTags = "",
    } = blog.seoInfo || {};

    const title = blog?.seoInfo?.seoTitle || seoTitle;
    const description = blog?.seoInfo?.seoDescription || seoDescription;
    const keywords = blog?.seoInfo?.seoKeywords || seoKeywords;
