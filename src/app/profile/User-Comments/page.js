...
import {
  ...
  editComment,
  getUserComments,
} from "@/services/comments/serviceComment";
import {
  CommentOutlined,
  DeleteOutlined,
  EditOutlined,
  ...
  const renderList = (items) => (
    <List
      ...
      renderItem={(item, index) => (
        <List.Item ref={index === items.length - 1 ? lastElementRef : null}>
          <Card
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      ...
      setTimeout(() => {
        if (activeTab === 0) {
          setComments((prev) =>
            prev.filter((item) => item.id !== selectedComment.id)
          );
            ...
            <div className="flex flex-wrap bg-white rounded-lg mt-3 z-50 relative">
              ...
              <div className="w-full">
                {loading && page === 1 ? (
                  ...
                  <Empty
                    description="شما هنوز پرسشی ثبت نکرده‌اید"
                    className="my-8"
                  />
                )}
              </div>
