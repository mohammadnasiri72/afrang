...
import {
  clearDiscount,
  setDiscountCode,
  setEstimateData as setDiscountEstimateData,
  setError,
  setLoading,
...
export default function SelectCodeDiscount() {
  ...
  const { discountCode, estimateData, isLoading, error } = useSelector(
  ...
  const handleInputChange = (e) => {
    setCode(toEnglishNumber(e.target.value));
    setInputError(false);
  };
          ...
          <form onSubmit={handleSubmit} className="space-y-2">
            <div className="flex gap-2">
              <input
                ...
                className={`w-full p-2 border rounded-lg focus:outline-none transition-colors duration-200 !text-[16px] ${
                  inputError
            ...
            <div>
              ...
              {estimateData?.discountCodeAmount > 0 && (
                <p className="text-gray-500 text-sm mt-1">
                  مبلغ تخفیف: {estimateData.discountCodeAmount.toLocaleString()}{" "}
                  تومان
                </p>
              )}
            </div>
            <Tooltip title="حذف کد تخفیف" arrow>
              <IconButton
